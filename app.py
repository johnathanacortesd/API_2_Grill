...
def _render_live_html(pct, msg, elapsed, file_label, active_key):
    ...
    <div class="live-panel">
      ...
      <div class="live-hint">Clustering anti-sobreagrupación activo: preserva la individualidad de cada hecho noticioso.</div>
...
def main():
    ...
    <div class="app-header">
        ...
        <div class="app-header-text">
            ...
            <div class="app-header-version">v3.2 · IA Precisión Sintáctica · Realizado por Johnathan Cortés</div>
        </div>
    ...
    if not st.session_state.get("processing_complete", False):
        ...
        with st.form("main_form"):
            ...
            c_enable, c_model = st.columns([2, 2])
            with c_enable:
                enable_ai = st.checkbox("Activar análisis reputacional con IA", value=True)
            with c_model:
                model_choice = st.selectbox(
                    "Modelo OpenAI para subtemas",
                    options=["gpt-4.1-mini", "gpt-4.1-nano-2025-04-14"],
                    index=0,
                    help="gpt-4.1-mini produce frases nominales más completas y sin truncamientos gramaticales."
                )
            ...
            with c_brand:
                brand_input = st.text_input(
                    ...
                    help="La IA evaluará el impacto reputacional respecto a esta marca."
                )
            with c_alias:
            ...
            st.markdown(
                ...
                "Si un eje no tiene PKL, se utiliza la IA. El subtema siempre se extrae con IA.</div>",
                unsafe_allow_html=True,
            )
            c_tono, c_tema = st.columns(2)
            with c_tono:
                f_tono = st.file_uploader("PKL de tono (opcional)", type=["pkl"], key="pkl_tono")
            with c_tema:
                f_tema = st.file_uploader("PKL de tema (opcional)", type=["pkl"], key="pkl_tema")
